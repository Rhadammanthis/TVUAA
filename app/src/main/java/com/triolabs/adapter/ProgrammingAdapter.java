...
public class ProgrammingAdapter extends BaseAdapter {
	...
	public ProgrammingAdapter(Context context,ArrayList<Program> listProgram){
		ProgrammingAdapter.listProgram =listProgram;
		ProgrammingAdapter.context=context;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(bmProgram==null){
			ImageAsyncTask task = new ImageAsyncTask(context,imageProgram,progressBar,program.getPhotoProgram(),Device.getWidth()/2,Device.getHeigth()/6);
			task.execute();
		}else{
		...
		convertView.setOnTouchListener(new OnTouchListener(){
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if(event.getAction() == MotionEvent.ACTION_DOWN){
	                //Button Pressed
					layoutText.setBackgroundColor(context.getResources().getColor(R.color.orange_low));
